...
model_name       = "/mnt/nvme2/yyc/TCM/TCM-BERT"
max_seq_length   = 512
...
learning_rate    = 5e-5
patience         = 3
output_dir       = "./tcmbert_classification_output"
...
# ------------- 模型加载 -------------
NUM_LABELS = 148
model = BertForTCMClassification.from_pretrained(model_name, num_labels=NUM_LABELS,ignore_mismatched_sizes=True)
model.to(device)
...
# ------------- 训练循环 -------------
for epoch in trange(int(num_train_epochs), desc="Epoch", mininterval=10):
    # 1) 根据当前阶段决定使用哪些难度的数据
    ...
    filtered = [s for s in train_samples
                if s.get("difficulty","easy") in current_difficulties]
    if not filtered:
        print("Warning: 本阶段无样本，退回为全量训练集")
    ...
    model.train()
    total_loss = 0.0
    for batch in tqdm(dl, desc=f"Training Epoch {epoch+1}",
                      leave=False, mininterval=10):
        b_ids, b_mask, b_tt, b_lbl = [x.to(device) for x in batch]
        model.zero_grad()
        outputs = model(b_ids, attention_mask=b_mask, labels=b_lbl)
        loss = outputs[0].mean()
        loss.backward()
        optimizer.step()
    ...
    print(f"Epoch {epoch+1} Dev → Loss: {val_loss:.4f}, Acc: {val_acc:.4f}, "
          f"Macro-F1: {val_macro_f1:.4f}, P: {val_prec:.4f}, R: {val_rec:.4f}")

    # 4) Macro-F1 增长率饱和判断（切换阶段／停止）、记录 history
    ...
    #             print(f"—— Macro-F1 增长饱和，切换到阶段 {difficulty_stages[stage_index]}")
    #         else:
    #             print("—— Hardest 阶段再度饱和，提前停止训练。")
    #             break

    # 5) Early stopping：patience-based :contentReference[oaicite:1]{index=1}
    if val_macro_f1 > best_macro_f1:
        # 有提升：更新 best，重置耐心计数，并保存模型
        best_macro_f1 = val_macro_f1
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(output_dir, "best_model.pt"))
        print("→ 验证集 Macro-F1 提升，保存最佳模型")
    else:
        if stage_index < len(difficulty_stages) - 1:
            # 前两阶段：不触发早停，直接切到下一个阶段，重置耐心计数
            stage_index += 1
            patience_counter = 0
            print(f"→ 阶段 {stage_index}（{difficulty_stages[stage_index]}）开始训练")
        else:
            # 最后一个阶段：才真正触发早停
            patience_counter += 1
            print(f"→ 最终阶段未提升，耐心计数: {patience_counter}/{patience}")
            if patience_counter >= patience:
                print("验证集 Macro-F1 连续未提升，Early stopping。")
                break
