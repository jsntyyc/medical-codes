...
model_name = "/mnt/nvme2/yyc/TCM/ZY-BERT"  # 使用ZY-BERT预训练模型
max_seq_length = 512
...
# ------------- 模型加载 -------------
NUM_LABELS = 148
# 这里采用 BertForTCMClassification 来加载 ZY-BERT
...
for epoch in trange(num_train_epochs, desc="Epoch", mininterval=30):
    ...
    for batch in tqdm(train_dataloader_epoch, desc=f"Training Epoch {epoch+1}", leave=False, mininterval=30):
        b_input_ids, b_attention_mask, b_token_type_ids, b_labels = [x.to(device) for x in batch]
        model.zero_grad()
        outputs = model(b_input_ids, attention_mask=b_attention_mask, labels=b_labels)
        loss = outputs[0]
