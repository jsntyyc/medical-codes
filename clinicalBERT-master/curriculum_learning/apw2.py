...
def run_apw_kmeans_bert(args):
    ...
    # 6. 全局中心二分 + 易例再聚类 → 得到三簇标签 clusters
    N = Z.shape[0]
    ...
    # 6.2 样本到中心距离
    dist_to_center = np.linalg.norm(Z - global_center[None,:], axis=1)

    # 6.3 按距离排序，取 top‐hard_ratio 为 hard，其余为初步 easy
    hard_ratio = rho_final
    hard_count = int(hard_ratio * N)
    sorted_idx = np.argsort(dist_to_center)[::-1]
    hard_idx = sorted_idx[:hard_count]
    easy_temp_idx = sorted_idx[hard_count:]

    # 6.4 初始化标签数组：2 表示 hard
    clusters = np.empty(N, dtype=int)
    clusters[hard_idx] = 2

    # 6.5 对剩下的 easy_temp 做 KMeans(2) ，得到两类易例
    Z_easy = Z[easy_temp_idx]
    km2 = KMeans(n_clusters=2, random_state=42).fit(Z_easy)
    easy_labels_temp = km2.labels_            # 0 或 1

    # 6.6 将这两类贴到 clusters 中，分别用 0、1 表示
    clusters[easy_temp_idx] = easy_labels_temp
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hard_ratio", type=float, default=0.2,
        help="最远 hard 样本占比，剩下先标记为 easy 再细分"
    )
    parser.add_argument("--data_dir", default="/mnt/nvme2/yyc/clinicalbert/data/discharge")
    parser.add_argument("--bert_model", default="/mnt/nvme2/yyc/clinicalbert/model/pretraining")
    parser.add_argument("--output_file", default="train_with_difficulty_0701_2_discharge.csv", help="输出 CSV 文件路径")
    ...
    parser.add_argument("--num_train_epochs", type=float, default=5.0, help="训练轮数")
