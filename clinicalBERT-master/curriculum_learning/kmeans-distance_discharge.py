...
def run_apw_kmeans_bert(args):
    ...
    # 6. 两簇聚类 & 簇内远距子集合并 → 得到三簇标签 clusters
    km2 = KMeans(n_clusters=2, random_state=42).fit(Z)
    init_labels = km2.labels_
    centers = km2.cluster_centers_
    # 在每簇内挑最远 farthest_ratio 的样本，合并成 hard
    clusters = np.empty(N, dtype=int)
    hard_idx = []
    for k in [0, 1]:
        idx_k = np.where(init_labels == k)[0]
        # 簇内到中心的欧氏距离
        d_k = np.linalg.norm(Z[idx_k] - centers[k], axis=1)
        # 取最远的 ⌈farthest_ratio * |簇k|⌉ 个样本
        num_hard_k = int(np.ceil(args.farthest_ratio * len(idx_k)))
        top_k = idx_k[np.argsort(d_k)[::-1][:num_hard_k]]
        hard_idx.extend(top_k.tolist())
    clusters[hard_idx] = 2

    for k in [0, 1]:
        idx_k = np.where(init_labels == k)[0]
        remain_k = list(set(idx_k) - set(hard_idx))
        clusters[remain_k] = k
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--farthest_ratio", type=float, default=0.15,
    ...
    parser.add_argument("--output_file", default="train_with_difficulty_kmeans+distance_discharge1.csv", help="输出 CSV 文件路径")
