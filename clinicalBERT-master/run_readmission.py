...
class readmissionProcessor(DataProcessor):# 数据读入与封装
    def get_train_examples(self, data_dir):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    # 1.WordPiece 分词 → tokens
    # 2.插入 [CLS]、[SEP]
    # 3.token_type_ids（全 0）+ attention_mask（1/0）
    # 4.padding 到 max_seq_length
    label_map = {}
...
def main():
    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            # 使用 tqdm 包装 train_dataloader 显示 step-level 进度
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {epo + 1}", leave=False)):
                ...
                # 遍历模型参数并打印梯度的范数（或直接打印梯度）
                # for name, param in model.named_parameters():
                #     if "classifier" not in name:
                #         param.requires_grad = False
                #     if param.grad is not None:
                #         print(f"{name} 的梯度范数: {param.grad.norm().item()}")
                #     else:
                #         print(f"{name} 没有梯度")
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(val_dataloader, desc=f"Validation Epoch {epo + 1}", leave=False):
